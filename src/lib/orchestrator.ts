...
import { runOpenCodeStep, waitForLoopContinuationIdle, type Step, type StepResult, type StepRunResult } from "./runner.ts";
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (result.status === "waiting" && result.sessionID !== undefined) {
        ...
        if (!skipLike) suppressFailureRetry = true;
        const activeStep = state.steps[index];
        if (activeStep) {
      ...
      if (
        result.status === "failed" &&
        !suppressFailureRetry &&
        failureRetryCount < MAX_FAILURE_RETRIES_PER_STEP &&
        !state.quitting &&
        !stopFileExists()
      ) {
        failureRetryCount += 1;
        ...
        if (hasOutput && priorSessionID !== undefined) {
          ...
          pushAgentLine(state, `[looper] ${step.name} failed (attempt ${failureRetryCount}/${MAX_FAILURE_RETRIES_PER_STEP}); resuming session ${priorSessionID}`);
          pushStepOutputLine(state, index, `[looper] ${step.name} failed (attempt ${failureRetryCount}/${MAX_FAILURE_RETRIES_PER_STEP}); resuming session ${priorSessionID}`);
        } else {
          resumeSessionID = undefined;
          resumePrompt = undefined;
          pushAgentLine(state, `[looper] ${step.name} failed (attempt ${failureRetryCount}/${MAX_FAILURE_RETRIES_PER_STEP}); restarting with a fresh session`);
          pushStepOutputLine(state, index, `[looper] ${step.name} failed (attempt ${failureRetryCount}/${MAX_FAILURE_RETRIES_PER_STEP}); restarting with a fresh session`);
        }
        const activeStep = state.steps[index];
        if (activeStep) {
          activeStep.status = "pending";
    ...
    if (result.status === "failed") throw new Error(`${step.name} failed after ${failureRetryCount} retr${failureRetryCount === 1 ? "y" : "ies"}`);

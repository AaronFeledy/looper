...
export function formatLooperEvent(event: LooperEvent): string[] {
  switch (event.kind) {
    case "step.started":
      return [groupHeader("OpenCode step", ui.cyan)];
    case "step.done":
      return [
    ...
    case "assistant.started":
      return [groupHeader("Assistant", ui.cyan)];
    case "assistant.text":
      return [event.text];
    ...
    case "reasoning.started":
      return [groupHeader("Reasoning", ui.magenta)];
    case "reasoning.text":
      return [`${ui.dim("│")} ${event.text}`];
    case "tool.started":
      return [`${ui.yellow("◌ tool")} ${ui.bold(event.tool)} ${ui.dim(formatInput(event.input))}`];
    case "tool.done": {
      const lines = [groupHeader(`Tool output · ${event.tool}`, ui.green), ...toolOutputLines(event.output)];

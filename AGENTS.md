...
- `src/lib/runner.ts` &mdash; runs one step against opencode (session create, prompt, subscribe to events, abort on cancel). Reads `.sisyphus/run-continuation/*.json` from the project repo to detect background tasks.

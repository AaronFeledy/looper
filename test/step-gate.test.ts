...
const GATE_CASES: readonly GateCase[] = [
  {
    ...
    inputs: { gate: {}, branch: undefined, storyId: undefined, phase: undefined },
    expected: { pass: true },
  },
  {
    name: "passes a story branch condition when a story id is derivable",
    inputs: { gate: { branch: "story" }, branch: "us-074-work", storyId: "US-074", phase: undefined },
    expected: { pass: true },
  },
  {
    name: "fails a story branch condition when the story id is underivable",
    inputs: { gate: { branch: "story" }, branch: "feature-work", storyId: undefined, phase: undefined },
    expected: { pass: false, reason: "gate: branch is not a story branch (current 'feature-work'; expected a name matching ^([a-z]+-[0-9]+[a-z]?)- or prefixed with an exact configured PRD story ID followed by '-')" },
  },
  {
    name: "fails a story branch condition when git has no branch",
    inputs: { gate: { branch: "story" }, branch: undefined, storyId: undefined, phase: undefined },
    expected: { pass: false, reason: "gate: branch is not a story branch (no current branch; expected a name matching ^([a-z]+-[0-9]+[a-z]?)- or prefixed with an exact configured PRD story ID followed by '-')" },
  },
  {
    name: "passes a main branch condition only on main",
    inputs: { gate: { branch: "main" }, branch: "main", storyId: undefined, phase: undefined },
    expected: { pass: true },
  },
  {
    name: "fails a main branch condition on a story branch",
    inputs: { gate: { branch: "main" }, branch: "us-074-work", storyId: "US-074", phase: undefined },
  ...
  },
  {
    ...
    inputs: { gate: { branch: "main" }, branch: undefined, storyId: undefined, phase: undefined },
    expected: { pass: false, reason: "gate: branch is not main (no current branch; expected 'main')" },
  },
  {
    name: "empty gate object passes (no conditions)",
    inputs: { gate: {}, branch: "us-074-work", storyId: "US-074", phase: undefined },
    expected: { pass: true },
  },
  {
    name: "treats a missing phase as building",
    inputs: { gate: { phase: "building" }, branch: "us-074-work", storyId: "US-074", phase: undefined },
    expected: { pass: true },
  },
  {
    name: "passes when the current phase equals the required phase",
    inputs: { gate: { phase: "verified" }, branch: "us-074-work", storyId: "US-074", phase: "verified" },
    expected: { pass: true },
  },
  {
    name: "passes when the current phase is beyond the required phase",
    inputs: { gate: { phase: "reviewed" }, branch: "us-074-work", storyId: "US-074", phase: "merged" },
    expected: { pass: true },
  },
  {
    name: "fails when the current phase is before the required phase",
    inputs: { gate: { phase: "verified" }, branch: "us-074-work", storyId: "US-074", phase: "reviewed" },
    expected: { pass: false, reason: "gate: phase reviewed is before verified (expected phase at or past verified)" },
  },
  {
    name: "phaseBelow fails open when no evaluated story is present",
    inputs: { gate: { phaseBelow: "implemented" }, branch: "main", storyId: undefined, phase: undefined },
    expected: { pass: true },
  },
  {
    name: "phaseBelow skips when the evaluated story is at the threshold",
    inputs: { gate: { phaseBelow: "reviewed" }, branch: "us-074-work", storyId: "US-074", phase: "reviewed" },
    expected: { pass: false, reason: "gate: phase reviewed is at or past reviewed (nothing to do)" },
  },
  {
    name: "phaseBelow skips when the evaluated story is past the threshold",
    inputs: { gate: { phaseBelow: "reviewed" }, branch: "us-074-work", storyId: "US-074", phase: "verified" },
    expected: { pass: false, reason: "gate: phase verified is at or past reviewed (nothing to do)" },
  },
  {
    name: "phaseBelow passes when the evaluated story is still below the threshold",
    inputs: { gate: { phaseBelow: "reviewed" }, branch: "us-074-work", storyId: "US-074", phase: "implemented" },
    expected: { pass: true },
  },
  {
    name: "phaseBelow treats a missing phase as building",
    inputs: { gate: { phaseBelow: "implemented" }, branch: "us-074-work", storyId: "US-074", phase: undefined },
    expected: { pass: true },
  },
  {
    name: "passes a script condition on exit zero",
    inputs: { gate: { script: "true" }, branch: undefined, storyId: undefined, phase: undefined, scriptResult: { ran: true, exitCode: 0 } },
    expected: { pass: true },
  },
  {
    name: "fails a script condition on nonzero exit",
    inputs: { gate: { script: "false" }, branch: undefined, storyId: undefined, phase: undefined, scriptResult: { ran: true, exitCode: 7 } },
    expected: { pass: false, reason: "gate: script exited with code 7" },
  },
  {
    name: "fails loudly when a script times out or cannot spawn",
    inputs: { gate: { script: "sleep 10" }, branch: undefined, storyId: undefined, phase: undefined, scriptResult: { ran: false, error: "timed out after 25ms" } },
  ...
  },
  {
    ...
    inputs: { gate: { script: "true" }, branch: undefined, storyId: undefined, phase: undefined },
    expected: { pass: false, reason: "gate: script did not run" },
  },
  {
    name: "AND-combines configured conditions and names the first failure",
    inputs: { gate: { branch: "story", phase: "verified", script: "true" }, branch: "main", storyId: undefined, phase: "building", scriptResult: { ran: true, exitCode: 0 } },
    expected: { pass: false, reason: "gate: branch is not a story branch (current 'main'; expected a name matching ^([a-z]+-[0-9]+[a-z]?)- or prefixed with an exact configured PRD story ID followed by '-')" },
  },
];
